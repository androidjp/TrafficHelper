...
class MainMenuPresenter implements MainMenuContract.Presenter {
    ...
    public void start() {
        ...
        final User user = UserManager.getInstance(mContext).getUser();
        if (user != null) {
            //TODO:登录
    ...
    public void checkUserLogin(int resultCode) {
        Logger.i("检查登录信息，resultCode = "+ resultCode);

        if (resultCode == Constants.FINISH_LOGIN) {
            if (this.mView.get() != null) {
                this.mView.get().refreshUserMsg(UserManager.getInstance(mContext).getUser());
            }
